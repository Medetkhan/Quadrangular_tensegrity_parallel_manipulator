...
double init_p1, init_p2, init_p3, init_p4;
double ref_p1, ref_p2, ref_p3, ref_p4;
...
int main(int argc, char **argv)
{
  ...
  std::cout <<i_p1<<" "<<i_p2<<" "<<i_p3 << i_p4 << std::endl;
    //motor positions
  double lp_r = 100;
  int ilp_r = lp_r;
  int fall_p = ilp_r*10;
  ...
  double amp4 = a * deg2rad; // Add amplitude for the fourth motor
  ...
  for(int i = 0; i!= size; ++i){
      ...
      pos4[i] = (amp4 * sin(w * i / size + 3 * phase));
  ...
  // std::cout <<pos1[i]<<" "<<pos2[i]<<" "<<pos3[i] << std::endl;
    ...
    double g_pos4 = (amp4 * sin(w * 0 /size + 3*phase));


static const int numberOfMotors = 4;
...
while(ros::ok() && count<size+fall_p*2) {
  if(count<calib_p){
    ...
    desiredConfiguration.positions[3] = g_pos4 + i_p4; // New line for the fourth motor
    motorCommand.joint_names[3] = "M4"; // New line for the fourth motor
    t_m = pos_t;
  }
  ...
  if (count>=calib_p && count<fall_p){
    ...
    desiredConfiguration.positions[3] = g_pos4 + i_p4; // New line for the fourth motor
    motorCommand.joint_names[3] = "M4";
    t_m = l_t;
  }

  if(count>=fall_p && count<(fall_p+size)){
    desiredConfiguration.positions[0] = pos1[count - fall_p] + i_p1;
    motorCommand.joint_names[0] = "M1";

    desiredConfiguration.positions[1] = pos2[count - fall_p] + i_p2;
    motorCommand.joint_names[1] = "M2";

    desiredConfiguration.positions[2] = pos3[count - fall_p] + i_p3;
    motorCommand.joint_names[2] = "M3";

    desiredConfiguration.positions[3] = pos4[count - fall_p] + i_p4; // New line for the fourth motor
    motorCommand.joint_names[3] = "M4"; // New line for the fourth motor
  ...
  if(count>=(fall_p+size)){
    desiredConfiguration.positions[0] = i_p1;
    motorCommand.joint_names[0] = "M1";

    desiredConfiguration.positions[1] = i_p2;
    motorCommand.joint_names[1] = "M2";

    desiredConfiguration.positions[2] = i_p3;
    motorCommand.joint_names[2] = "M3";

    desiredConfiguration.positions[3] = i_p4; // New line for the fourth motor
    motorCommand.joint_names[3] = "M4"; // New line for the fourth motor
